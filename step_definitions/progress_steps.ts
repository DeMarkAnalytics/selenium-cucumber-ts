...
Then(
  ...
  async function (this: World, seconds: string, elementType: string, typeValue: string) {
    await progress.waitForElementToDisplay(this, elementType, typeValue, +seconds);
  }
);
...
Then(
  ...
  async function (this: World, seconds: string, elementType: string, typeValue: string) {
    await progress.waitForElementToBeLocated(this, elementType, typeValue, +seconds);
  }
);

Then(/^I (?:should\ )?wait (\d+) seconds for the page title to be "(.*?)"$/, async function (this: World, seconds: string, titleMatch: string) {
